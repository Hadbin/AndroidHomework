...
public class MemoDao {
    ...
    public int insertMemo(Memo memo){
        ...
        db.execSQL("insert into memosTable (title,content,createTime,alarmTime,isAlarm,groupId)"
    ...
    public int updateMemo(Integer mId,Memo memo){
        SQLiteDatabase db=new DBHelper(
                context, "memo.db", null, 2).getWritableDatabase();
        db.execSQL("update memosTable set title=? ,content=? ,createTime=? ,alarmTime=? ,isAlarm=? ,groupId=? where mId=?",
                new String[]{
                        memo.getTitle(),memo.getContent(),memo.getCreateTime(),memo.getAlarmTime()
                        ,String.valueOf(memo.getIsAlarm()),String.valueOf(memo.getGroupId()),String.valueOf(mId)
                });
        return 1;
    }
